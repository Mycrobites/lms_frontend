...
const sendTask = (url, task) => {
  const result = ax.post(url, task).catch((err) => {
    throw err;
  });
  return result
    .then((result) => {
...
export const updateTask = (taskId, task) => {
  ax.put(`edit/${taskId}`, task).catch((err) => {
    throw err;
  });
};
...
class Task extends Component {
  ...
  componentDidMount() {
    ...
    this.state.todoList.map((obj) => ({ ...obj, isEditing: "false" }));
  }
  ...
  render() {
    ...
    const editClick = (taskId, TorF) => {
      ...
      if (
        !TorF &&
        this.state.editTaskTitle !== "" &&
        this.state.editTaskDate !== ""
      ) {
        const todo = {
          ...
          {this.state.todoList.map((task) =>
            !task.isEditing ? (
              <div key={task.id} className="Task">
                ...
                <div>
                  <EditRoundedIcon onClick={() => editClick(task.id, true)} />
                </div>
                <div>
                  <DeleteIcon onClick={() => deleteHandle(task.id)} />
                </div>
              </div>
            ) : (
              <div key={task.id} className="Task">
                ...
                <div>
                  <button onClick={() => editClick(task.id, false)}>
                    Edit
                  </button>
                </div>
              </div>
