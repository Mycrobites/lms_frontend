...
const PastQuiz = ({ attemptedQuiz, missedQuiz }) => {
  return (
    <div className="past-quiz">
      ...
      { attemptedQuiz.length === 0 && missedQuiz.length === 0 ? <div className='quiz-div no-header'><h5>No past quiz available .</h5></div> :
        ...
        attemptedQuiz?.map((quiz) => {
          ...
          return (
            <div key={id} className="single-quiz">
              ...
              <button style={{opacity:".7", cursor:"not-allowed"}}>Attempted</button>
            </div>
          );
        })}
        ...
        {missedQuiz?.map((quiz) => {
          ...
          return (
            <div key={id} className="single-quiz">
              ...
              <button style={{opacity:".7" , cursor:"not-allowed"}}>Missed</button>
            </div>
          );
        })}
      </div> }
